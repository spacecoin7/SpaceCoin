...
#if defined(NDEBUG)
# error "Spacecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe9a6385e7409e35f6f6e889a2dd3b760134343f9510ef776a8939f1c3752928c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Spacecoin: starting difficulty is 1 / 2^12
...
{
    // Spacecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Spacecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan =  10 * 60; // Spacecoin: 10 minutes
static const int64 nTargetSpacing = 1 * 30; // Spacecoin: 0.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Spacecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Spacecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe9a6385e7409e35f6f6e889a2dd3b760134343f9510ef776a8939f1c3752928c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC News Wed 14 Jan 2015 Rush to buy new Charlie Hebdo issue";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("vxTPvcYjwVazTpMMxXBcFRgtQzt5uW2g3TEwMesF3sJecKoTsFLLWJCzzye6SWK8PJn5VgRkUFo56GiH9LxBYMBn6aFrvnUkDW7W6mFSs4XbJfzcCLKAXyTewpUbLXwjeo") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1421242705;
        ...
        assert(block.hashMerkleRoot == uint256("0x529aada8cce1dde395b39ad43ca5a7412c311a5213d1ee252431fe0eb88cc3a3"));
...
unsigned char pchMessageStart[4] = { 0x78, 0x73, 0x63, 0xdb }; // Spacecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SpacecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SpacecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SpacecoinMiner(CWallet *pwallet)
{
    printf("SpacecoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running SpacecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SpacecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SpacecoinMiner, pwallet));
}
